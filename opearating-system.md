...
1. 如果需要利用多核提升性能，可以采用多线程的方法。
2. 如果线程之间需要共享的变量内容过大（例如一个proxy需要缓存接收到的内容，此时需要共享的内存区域较大），可以考虑使用多线程。
...
内核为每个进程都维护了一个上下文，上下文保存了内核为了恢复一个正在运行的进程所需要的全部状态。通常，进程上下文不仅包含了用户栈、全局变量等用户态空间资源，程序计数器、通用寄存器等CPU上下文内容，还包括括了内核栈、内核数据结构（页表、文件表等）等内核空间资源。
...
clone提供了一种更加精细化的控制，可以由用户决定父子进程之间共享哪些区域，通过flags参数控制进程（线程）之间的共享程度。Linux的进程创建和线程创建都用到了clone。
