...
4. 线程同样具有状态之间的转换关系，线程能减少并发执行的时间和空间开销。
...
除此之外，进程和线程之间的实现没有本质区别，Linux线程是一个轻量级的进程。
...
* 如果两个线程属于一个进程，由于共享地址空间，切换过程中虚拟内存之类的资源保持不变，只需要切换线程的私有数据、寄存器等不共享变量。同时，由于地址空间不变，TLB表中的表项还是有效的，避免了TLB的失效，速度更快。

#### Linux中的实现步骤

Linux中的上下文切换主要包括以下两个步骤：

* 切换页全局目录，切换为一个新的地址空间。
* 切换内核态堆栈和硬件上下文（硬件上下文提供了执行新进程需要的所有信息），用于执行新的进程。
...
每个进程按照优先级分配一个范围的彩票，彩票对应分配的份额。调度时，会根据随机数（彩票开奖）确定任务是否被调度。一个任务占用的份额越大，开奖的彩票更有可能落在它的份额内。

采用随机数会带来一个问题：只有调度次数足够多，彩票调度的效果才接近公平。对于运行时间较短的任务，如果调度次数过少，实际分配的CPU份额可能因为随机数与预期的不同，不能保证公平。

##### 步幅调度

步幅调度是一种确定性的公平调度。步幅调度同样使用份额的概念，表示每个任务分配的CPU时间比例。不同点在于引入了虚拟时间的概念（步幅）。

虚拟时间与真实的物理时间没有直接的关联，使用虚拟时间的调度系统一般选择调度虚拟时间最小的任务。每个任务都会记录自己的虚拟时间，并且只有在自己的任务被调度时才增加虚拟时间的长度。每个任务都使用相同的时间片。运行了相同的时间片后，虚拟时间增加的比例和步幅成正比。通过步幅计算的虚拟时间，可以保证任务被调度的次数和份额成正比。经历了相同虚拟时间的任务，它们使用CPU时间之比就是步幅的反比。
...
CFS使用的调度策略类似于步幅调度，使用vruntime概念表示步幅调度中的步幅。同时，设置了nice值和权重的对应关系，nice值越低，对应的任务权重越高。

 相较于其他的调度方法，CFS有以下的特点：

* 不同于其他的调度算法需要一个固定的时间片大小，CFS使用vruntime（虚拟运行时间）表示每个进程的运行时间，达到公平划分CPU时间的目的。每次进程调度的时候选择vruntime最小的进程运行。所有进程vruntime的增长速率是一致的。操作系统利用sched_latency（一般为48ms）计算每个进程的时间片，每个进程的时间片基于nice值的不同而不同。同时，操作系统通过参数min_granularity保证一个进程最少需要运行多久（一般为6ms）。
* 通过nice值控制所有进程的调度权重。通过sched_latency结合nice值加权的方式求出每个进程应该得到的时间片，vruntime通过以前的vruntime、真实的运行时间以及nice值进行更新，保证了不同权重的进程vruntime增长速率是一致的。
...
进程间上下文切换的时候需要保证TLB的内容和当前页表的内容是一致的。一种思路是在上下文切换时直接清空TLB中的内容，进程不会读到错误的地址，但是会造成一定程度的开销（会触发TLB不命中）。如果频繁进行上下文切换，开销很大，会造成性能损失。有些系统通过硬件支持避免这种情况，实现跨进程上下文的TLB共享。一种做法是采用地址空间标识符缓存不同进程的地址空间映射，如x86-64上的PCID（Process Context IDentifier）。
...
* 使用页目录表项告知使用页表的页或者整个页的页表是否包含有效页。
...
如果程序访问的虚拟地址对应的物理地址不存在，就需要通过操作系统处理。这种错误称为页错误。遇到页错误时，操作系统需要调用缺页处理程序处理，也就是缺页异常处理程序。

操作系统需要知道虚拟地址对应的物理页的磁盘地址，这样才能在处理缺页错误时找到物理页并从磁盘加载到内存中。

为了处理页错误，操作系统采取以下的流程（又称为页面换入）：
...
* 将物理页面的内容写入到磁盘中。
* 更新页表，解除虚拟页和物理页的映射，同时记录物理页在磁盘上写入的对应位置。
...
通常，换入页面需要涉及到大量的磁盘操作，操作系统需要引入预取机制解决进行优化。当操作换入操作时，预测还有哪些页面会被访问，提前将这些页面换入到物理内存，减少发生缺页异常的次数。
...
实现近似LRU的可行方法：任何周期性地清除使用位，然后通过1和0判断替换哪个页的方法。

时钟算法是一种近似LRU算法。开始时，时钟指向一个页面，当操作系统需要执行页面替换的操作时，如果指向的页面的使用位是1，就把页面的使用位置为0，随后指针指向下一个页面。指针持续转动，直到找到一个使用位为0的页面。最终选择该页执行页面替换操作。
...
slab用于内核数据结构的分配和释放，可以加快内存的分配和释放速度，同时避免了空闲链表不能全局控制的问题。针对的对象时频繁分配和释放的内存数据结构，例如进程控制块、inode。slab相当于内核通用数据结构的缓存层。slab通常作为小内存的分配机制，将伙伴系统分配的内存进行进一步的分配。
...
* 空闲列表：用于指示磁盘中的数据块是空闲的还是被使用的，用于写入文件时分配新的数据块。空闲列表又分为数据块空闲列表（保存数据块的空闲信息）和inode表空闲列表（保存inode表中的空闲信息）。
* 超级块：用于存储文件系统的信息。



inode中最重要的设计是如何引用数据块的位置，通常有以下几种做法：
...
系统为了记录哪个数据块和inode是空闲的，在分配新目录或者文件时就可以很快找到空闲的空间。常见的做法是使用位图法。
...
* 目录项对象：代表一个目录项，是路径的一个组成部分。路径的每一个组成部分都是一个文件。虽然可以使用索引节点对象表示每一个文件，但是为了方便查找，内核为每一个路径上的文件创建一个目录项对象。目录项纪录了文件的名字、索引节点指针以及与其他目录项的层级关联关系。目录项由内核创建并维护，不用写到磁盘上。
...
1. 应用程序调用操作系统提供的文件系统相关的系统调用。
2. 系统调用调用虚拟文件系统提供的接口，完成预期的功能。
3. 虚拟文件系统根据系统调用的内容和上下文，决定调用某个具体的文件系统的接口。

#### 页缓存

直接访问存储设备存在两个问题：

1. 目前大多数存储设备都是块接口，然而文件系统的更新并非都是对齐到块的边界，读取的字节数也并非是块大小的整数倍。
2. 存储设备的访问速度很慢，大量频繁的IO操作会成为性能瓶颈。

因此，文件系统使用内存作为缓存。缓存分为读缓存和写缓存。

读缓存基于时间局部性，当文件系统读取了某个文件的数据后，可以让这个数据在内存中再继续保留一段时间。读缓存需要消耗内存空间，操作系统会限制读缓存的大小，并且通过LRU算法回收读缓存的内存空间。

写缓存主要为了解决的问题是：如果每个写请求都需要等待存储设备的写入，文件写操作的延时会变的很大，吞吐量降低，影响性能。因此，在文件系统设计时做出了一个权衡，文件写请求返回后，操作系统允许修改的内容暂时不写入磁盘中。这样做保证了写入的性能，但是会牺牲了一定的可靠性。操作系统为了确保数据被持久化到磁盘上，提供了fsync接口。

##### Linux页缓存

Linux内核中的读缓存和写缓存采用统一的方式管理。

当读取一个文件时，系统首先检查内容是否已经保存在了页缓存中，如果是的话，直接从页缓存中读取对应的内容，返回给应用程序；如果不是的话，文件系统会创建一个新的页缓存，并从磁盘中读取相关数据，保存在页缓存中。

当执行文件写入的操作时，文件系统会把页缓存中修改过的页标记为“脏页”，加入到脏页列表中，由一个进程（回写进程）周期性地将脏页列表的页写回到磁盘中。这种策略称为“回写”。
