...
2. 进程之间的地址空间时独立的，一个进程的所有线程共享一个地址空间。
3. 
4. 线程之间可以通过全局变量通信，而进程之间需要使用IPC通信。
...
slab用于内核数据结构的分配和释放，可以加快内存的分配和释放速度，同时避免了空闲链表不能全局控制的问题。针对的对象时频繁分配和释放的内存数据结构，例如进程控制快、inode。
