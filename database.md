...
使用聚集索引，可以在索引的数据页上直接找到数据，加快范围查询速度。并且，针对主键的范围查找以及排序操作也特别快。

聚集索引中数据排列顺序有序指的是逻辑上有序，通过双向链表组织，不一定在物理存储上连续。
...
MySQL（5.0版本以后）可以使用索引合并利用多个单列索引定位需要的行，但是存在几个问题：
...
* 幻读：一个事务在前后两次查询同一个范围的时候，后一次查询看到了前一次查询没有看到的行。
  ...
  > 这种问题在当前读情况下才会出现。普通的一致性非锁定读是快照读，显式加锁的方式是当前读。
...
* 乐观锁的设计思路如下：如果可能发生前在的冲突，事务会继续执行而不是终止，寄希望于一切相安无事。当提交的时候，检查是否确实发生了冲突，如果发生了冲突，终止事务并重试。适用于读多写少的场景。

悲观锁的代表算法是两阶段封锁协议和基于时间戳的并发控制，乐观锁的代表算法是基于有效性检查的协议。

#### 两阶段封锁协议

两阶段锁协议要求事务分两个阶段提出加锁和解锁请求：

* 增长阶段：可以获得锁，但不能释放锁。
* 缩减阶段：可以释放锁，但不能获得锁。

两阶段封锁协议可以保证整个调度是串行化的，但是不能避免死锁的产生。

#### 基于时间戳的并发控制

对于系统中的每一个事务T，都用一个时间戳关联起来，可以通过两种手段实现：
...
在可重复读的隔离级别下，普通的select语句就是快照读，读取的是快照的数据。快照读不需要加锁的操作。
...
数据库区分快照读和当前读，避免了普通的select需要加锁的操作，提升了并发读的效率。