...
1. 数据不仅保存在叶子结点上，还会保存在内部节点中。

B+-树的特点：

1. 数据只保存在叶子结点上，内部节点只包含指向下一个页面的指针。
...
InnoDB对于行的查询使用的都是Next-Key Lock，设计的目的为解决幻读问题。存在一个特例，如果在唯一索引上执行等值查询，Next-Key Lock会退化为Record Lock。
