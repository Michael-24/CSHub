...
2. 服务器从客户端的密码套件列表中选择一个对称算法、一个密钥加密算法、一个哈希算法（验证摘要是否被篡改）。为了验证自己的身份。服务端会把自己的数字证书发送给客户端。服务器同时选择一个随机数S发送给客户端。最后，服务器使用私钥生成摘要发送给客户端。
...
TCP keep-alive提供了一种心跳机制，检测客户端或者服务端是否已经断开连接。
