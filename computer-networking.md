...
1. DHCP服务器发现。客户端发送DHCP发现报文，发现可以与其交互的DHCP服务器。发现报文的目的地址是广播地址255.255.255.255（主机不知道自己所处网络的IP地址），目的端口是67，源地址是0.0.0.0，源端口是68。链路层会把这个报文广播给所有的主机。
...
* 由于HTTP工作在TCP协议之上，因此需要建立TCP连接（三次握手）。
...
一般的网站会返回301重定向响应报文。但是如果简单使用302重定向报文，有可能会有被中间人劫持的风险。可以使用HSTS，支持 HSTS 的服务端，可以强制访问它的浏览器使用 HTTPS 协议。 可以最大限度的减少 HTTP 协议的各种安全问题了。
...
第三次握手丢失：客户端把自己的状态转为established，但是服务端并没有。服务端在重传SYN+ACK到达指定上限后释放了维护连接的资源。当客户端向服务端
