...
如果一台主机需要找到同一个子网下的某个主机的IP地址，过程如下：
...
1. DHCP服务器发现。客户端发送DHCP发现报文，发现可以与其交互的DHCP服务器。发现报文的目的地址是广播地址255.255.255.255（主机不知道自己所处网络的IP地址），源地址是0.0.0.0。链路层会把这个报文广播给所有的主机。
...
* 主机执行域名解析。首先从浏览器和缓存文件中查找目标主机的IP地址，如果这个地址不存在，使用DNS协议获得目标主机的IP地址。如果主机不知道网关路由器的地址，
...
* Cookie采用ASCII码形式保存，Session采用二进制形式保存。
* 
...
一般的网站会返回301重定向响应报文。但是如果简单使用302重定向报文，有可能会有劫持的风险。
...
第二次握手丢失：客户端重传SYN，服务端也会重传SYN+ACK。

第三次握手丢失：客户端把自己的状态转为established，但是服务端并没有。服务端在重传SYN+ACK到达指定上限后释放了维护连接的资源。

### 如何选择初始序列号？

TCP根据连接双方的IP地址和端口号计算初识序列号。如果连接的初识序列号是固定的，会让黑客更容易攻击
...
* 服务端处理连接需要耗费特别长的时间，导致客户端连接超时并断开连接；或者客户端连接超时时间设置过短，服务端还没有来的届处理就已经连接超时。这种情况下需要检查服务端是否存在长时间阻塞的代码模块，并尝试使用非阻塞或者异步的方式实现。
...
超时重传是TCP保障可靠传输的一种方法，如果一个已发送报文在超时时间内没有收到ACK，那么就重传这个报文段。
