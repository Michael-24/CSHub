...
MTU：最大传输单元。表示一条链路的最大帧长度，这是数据链路层的概念。

MSS：最大报文段。传输层一次从缓存中发送的最大数据量受限于此，这是传输层的概念。
...
2MLS的时间是从客户端接收到FIN后发送ACK开始计时的，如果在等待过程中，客户端又收到了一个FIN报文，那么需要重新开始计时。
...
TCP利用累计ACK机制，允许延迟一段时间发送ACK包，将ACK和同一方向上的需要发送的数据结合发送，这种捎带发送的数据适用于批量传输。采用Delay ACK能在一定程度上减小ACK传输数目，可以在一定程度上减轻网络负担。
...
基于窗口的流量控制机制会出现糊涂窗口综合征的缺陷。此时，交换的数据报的大小不是窗口大小的长度，而是一些较小的数据段，有用数据比例更小，耗费的资源更多，相应的传输效率更低。
